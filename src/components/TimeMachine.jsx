...
export default function TimeMachine({ isOpen, onClose, stats = {}, activities = [], chatHistory = [], meetings = [], documents = {}, onReopenItem }) {
  ...
  if (documents.invoice) {
    historyItems.push({
      id: 'doc-invoice',
      type: 'financial',
      title: `Invoice Generated: ${documents.invoice.invoiceNo || 'INV-TEMP'}`,
      subtitle: `Client: ${documents.invoice.client || 'Unknown'} | Total Due: $${documents.invoice.total ? documents.invoice.total.toLocaleString() : '0'}`,
      dateText: documents.invoice.date || 'July 6, 2026 15:00',
  ...
  if (documents.quotation) {
    historyItems.push({
      id: 'doc-quotation',
      type: 'financial',
      title: `Quotation Prepared: ${documents.quotation.quoteNo || 'QT-TEMP'}`,
      subtitle: `Client: ${documents.quotation.client || 'Unknown'} | Gross Offer: $${documents.quotation.total ? documents.quotation.total.toLocaleString() : '0'}`,
  ...
  if (documents.monthly_report) {
    historyItems.push({
      id: 'doc-report',
      type: 'report',
      title: `Sales Performance Report: ${documents.monthly_report.title || 'Performance Report'}`,
      subtitle: `Revenue Tallies: ${documents.monthly_report.revenue || '0'} | Efficiency Utility: ${documents.monthly_report.utility || '0%'}`,
